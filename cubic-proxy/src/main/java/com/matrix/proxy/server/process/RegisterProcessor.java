...
public class RegisterProcessor extends DefaultMessageProcess {
    ...
    public void process(ChannelHandlerContext ctx, String message) {
        ...
        builder.lastHeartbeat(new Date()).startDate(new Date()).progress(osInfo.get("process_no")).host(osInfo.get("host_name")).ip(osInfo.get("ipv4")).language(osInfo.get("language")).os(osInfo.get("os_name"));
